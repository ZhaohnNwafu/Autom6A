...
class Agent:
    def __init__(self,
        ...
        self.local_model_engines = ['codellama-7bi','codellama-13bi','llama2-13bc',
                                    'deepseek-6.7bi','deepseek-7bi']
        self.gpt_model_engines = ['gpt-3.5-turbo','gpt-4-turbo','gpt-4', 
        ...
        # RAG 初始化
        if self.rag:
            self.retriever = preload_retriever(True, None,
        ...
        if self.rag and self.retriever:
            self.log("[INFO] RAG已启用，正在测试检索功能...")

            # 根据数据类型选择不同的测试查询
            if self.data_type == "Nanopore":
                test_query = "Nanopore direct RNA sequencing m6A modification detection workflow"
            else:
                test_query = "MeRIP-seq analysis workflow"
        
            # 测试检索
            from src.build_RAG_private import retrive
            test_result = retrive(
                self.retriever,
                retriever_prompt=test_query,
                top_k=1,
                verbose=False  # 设为True可以看详细信息
            )
            
            if len(test_result) > 100:
                self.log(f"[INFO] RAG测试成功，检索到 {len(test_result)} 字符的内容")
                self.log(f"[INFO] 内容预览: {test_result[:200]}...")
                if self.data_type == "Nanopore":
                    self.log("[INFO] Nanopore专用知识库已加载")
            else:
                self.log(f"[WARNING] RAG检索结果太短（{len(test_result)}字符），可能知识库为空")
                self.log("[WARNING] 请检查 softwares_database/ 目录是否包含文档")
        else:
            self.log("[INFO] RAG未启用")
        ...
        if self.model_engine.startswith('ollama_'):
            self.log('[INFO] using ollama engine!')
        elif self.model_engine not in self.valid_model_engines:
            self.log('[ERROR] model invalid, please check the model engine selected!')
            exit()

        # use gpt model
        if self.model_engine in self.gpt_model_engines:
            # use deepseek online model 
            if self.model_engine=='deepseek-reasoner' or self.model_engine=='deepseek-chat':
        ...
        if 'llama' in self.model_engine and 'ollama_' not in self.model_engine:
            ...
            if self.model_engine == 'codellama-7bi':
                self.local_llm_generator = api_preload(ckpt_dir='src/codellama-main/CodeLlama-7b-Instruct-hf/',
                                        tokenizer_path='src/codellama-main/CodeLlama-7b-Instruct-hf/tokenizer.model',
                                        max_seq_len=4096)
    ...
    def get_single_response(self, prompt):
        ...
        if self.model_engine in self.gpt_model_engines:
            ...
            elif self.model_engine=='deepseek-reasoner' or self.model_engine=='deepseek-chat':
                response = self.openai_client.chat.completions.create(
                    model=self.model_engine,
                    response_format={"type": "json_object"},
                    messages=[
    ...
    def execute_code(self, response_message):
        ...
        try:
            ...
            if self.execute:
                ...
                # 传递stop_flag给executor
                executor_info = self.code_executor.execute(
                    bash_code_path=f'{self.output_dir}/{self.global_round}.sh',
                ...
                # 检查executor_info是否表示被中断
                if executor_info and "interrupted" in str(executor_info).lower():
                    self.log("[EXECUTION INTERRUPTED]")
                    raise InterruptedError("Task interrupted during execution")
                ...
                if len(executor_info) == 0:
                    ...
                    self.log('[CHECKING EXECUTION RESULTS]')
                    if 'llama' in self.model_engine or 'deepseek-7bi' in self.model_engine:
                    ...
                    while not self.valid_json_response_executor(executor_response_message):
                        ...
                        self.log(f'[EXECUTOR RESPONSE CHECKING TEST #{n_tries}/{max_tries}]')
                        if 'gpt' in self.model_engine or 'deepseek-reasoner' in self.model_engine or 'deepseek-chat' in self.model_engine:
                            time.sleep(20)
    ...
    def run_plan_phase(self):
        ...
        # 在开始规划阶段时
        self.log("\n" + "="*50)
        self.log("Phase 1: Planning Stage")
        self.log("="*50 + "\n")
        ...
        if self.gui_mode:
            ...
            while not self.valid_json_response(response_message):
                ...
                self.log(f'[Invalid Response, Waiting for 20s and Retrying...]')
                self.log(f'[INVALID RESSPONSE]\n{response_message}')
                if 'gpt' in self.model_engine or 'deepseek-reasoner' in self.model_engine or 'deepseek-chat' in self.model_engine:
                    time.sleep(20)
                response_message = self.get_single_response(init_prompt)
            ...
            with Spinner(f'\033[32m[AI Thinking...]\033[0m'):
                ...
                while not self.valid_json_response(response_message):
                    if self.check_stop():
                        raise InterruptedError("Task interrupted by user")
                    self.log(f'\033[32m[Invalid Response, Waiting for 20s and Retrying...]\033[0m')
                    self.log(f'[INVALID RESSPONSE]\n{response_message}')
                    if 'gpt' in self.model_engine or 'deepseek-reasoner' in self.model_engine or 'deepseek-chat' in self.model_engine:
                        time.sleep(20)
                    response_message = self.get_single_response(init_prompt)
    ...
    def run_code_generation_phase(self):
        self.log("\n" + "="*50)
        self.log("Phase 2: Code Generation & Execution")
        self.log("="*50 + "\n")
        ...
        while len(self.tasks) > 0:
            ...
            while self.execute_success == False:
                ...
                # execute code
                if self.gui_mode:
                    self.log('[AI Executing codes...]')
                    self.log(f'[Execute Code Start]')
                    ...
                    self.log(f'[Execute Code Finish],{self.execute_success}, {self.execute_info}')

                    if self.execute_success:
                        self.log(f'[Execute Code Success!]')
                        self.execute_info = ''
                    else:
                        self.log(f'[Execute Code Failed!]')
                        self.first_prompt = False
                else:
                    with Spinner(f'\033[32m[AI Executing codes...]\033[0m'):
                        self.log(f'\033[32m[Execute Code Start]\033[0m')
                        execute_success, execute_info = self.execute_code(response_message)
                        self.execute_success = execute_success
                        self.execute_info = execute_info
                        self.log(f'\033[32m[Execute Code Finish]\033[0m, {self.execute_success}, {self.execute_info}')

                        if self.execute_success:
                            self.log(f'\033[32m[Execute Code Success!]\033[0m')
                            self.execute_info = ''
                        else:
                            self.log(f'\033[31m[Execute Code Failed!]\033[0m')
                            self.first_prompt = False
    ...
    def run(self):
        try:
            ...
            self.log(f'[Job Finished! Cheers!]')
        except InterruptedError:
            self.log('\n⚠️ 任务已被用户中断')
            self.log('✅ 可以继续对话或开始新任务')
        except Exception as e:
            self.log(f'\n❌ 任务执行出错: {e}')
            import traceback
